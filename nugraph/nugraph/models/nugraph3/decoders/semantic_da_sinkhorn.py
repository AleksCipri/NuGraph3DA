"""NuGraph3 semantic decoder with domain adaptation (Sinkhorn alignemnt)"""
...
class SemanticDecoderDAsinkhorn(nn.Module):
    """
    ...
    If turned on, use Sinkhorn alignement loss on planar node embedding 
    ...
    def forward(self, dataS: Data, dataT: Data, stage: str = None) -> dict[str, Any]:
        ...
        if self.use_domain_adaptation:
            """
            Domain Adaptation is optional and is implemented via Sinkhorn loss (lossDA).
            ...
            raw_lossDA = wDA * self.loss_sinkhorn(dataS["evt"].x_semantic, dataT["evt"].x_semantic, blur=max(dynamic_blur_val, 0.01)) + self.temp_DA  
