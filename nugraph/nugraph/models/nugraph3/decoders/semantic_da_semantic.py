"""NuGraph3 semantic decoder with domain adaptation (Semantic alignemnt)"""
...
class SemanticDecoderDAsemantic(nn.Module):
    """
    ...
    If turned on, use Semantic alignement loss on planar node embedding 
    ...
    def __init__(self,
        ...
        # #option to add extra hidden layer to use those embeddings for alignemnt.
        # for p in planes:
        #     self.net[p] = nn.Sequential(
        #     nn.Linear(node_features, hidden_dim),
        #     nn.ReLU(),
        #     nn.Linear(hidden_dim, len(semantic_classes))
        # )
        #     self.classes = semantic_classes
    ...
    def forward(self, dataS: Data, dataT: Data, stage: str = None) -> dict[str, Any]:
        ...
        if self.use_domain_adaptation:
            """
            Domain Adaptation is optional and is implemented via Semantic DA alignment loss (lossDA) 
            i.e., minimizing Euclidean or Cosine distance for same labels. 
            ...
            raw_lossDA = wDA * self.loss_semantic(dataS[p].x_semantic, yS, dataT[p].x_semantic, yT) + self.temp_DA #should likely use xS and xT because those are stacked?
