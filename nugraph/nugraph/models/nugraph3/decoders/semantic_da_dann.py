"""NuGraph3 semantic decoder with domain adaptation (DANN)"""
...
class SemanticDecoderDAdann(nn.Module):
    """
    ...
    If turned on,  use Domain Adversarial Neural Network (DANN) loss on planar node embedding 
    ...
    def __init__(self,
        ...
        self.loss_dann = nn.CrossEntropyLoss()  # Optional Domain Adapttaion - DANN
        ...
        self.domain_net = nn.Linear(in_features=node_features,
                             out_features=2)
    ...
    def forward(self, dataS: Data, dataT: Data, stage: str = None) -> dict[str, Any]:
        ...
        if self.use_domain_adaptation:
            ...
            for p in self.net:
                reversed_S = ReverseLayerF.apply(dataS[p].x_semantic, alpha)
                xSS = self.domain_net(reversed_S)
                xSS_list.append(xSS)
        
                reversed_T = ReverseLayerF.apply(dataT[p].x_semantic, alpha)
                xTT = self.domain_net(reversed_T)
                xTT_list.append(xTT)
        
            xSS_all = torch.cat(xSS_list, dim=0)  #combine all planes, i.e. there is only one adversarial branch for combined semantic features
            ...
            combined_image = torch.cat((xSS_all, xTT_all), dim=0)  # merge source and target
