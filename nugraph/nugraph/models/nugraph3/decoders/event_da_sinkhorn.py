"""NuGraph3 event decoder with domain adaptation (Sinkhorn)"""
...
class EventDecoderDASinkhorn(nn.Module):
    """
    ...
    Use Sinkhorn loss on interaction node embedding to align them across the source and target dataset and improve classification.
    ...
    def forward(self, dataS: Data, dataT: Data, stage: str = None) -> dict[str, Any]:
        ...
        if self.use_domain_adaptation:
            """
            Domain Adaptation is implemented via Sinkhorn alignment (lossDA). 
            ...
            raw_lossDA = wDA * self.loss_sinkhorn(dataS["evt"].x, dataT["evt"].x, blur=max(dynamic_blur_val, 0.01)) + self.temp_DA  #Lower bound for blur
